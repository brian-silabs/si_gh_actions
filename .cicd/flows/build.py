...
def main() -> None:
    ...
    # NOTE: This is generated by `slt install` into the repo CI area (as in your original script).
    autogen_slconf = cicd_dir / "autogen" / "pkg.slconf"

    # Project-provided slconf used by slc generate (may or may not be the same file; keep explicit)
    project_slconf = cicd_dir / "user.slconf"  # adjust if your repo stores it elsewhere
    ...
    # ------------------------------------------------------------------
    # SLT install (generates .cicd/autogen/pkg.slconf)
    # ------------------------------------------------------------------
    env0 = dict(os.environ)

    # Ensure autogen directory exists before install (some tools expect it)
    (cicd_dir / "autogen").mkdir(parents=True, exist_ok=True)

    if not autogen_slconf.exists():
        log(
    ...
    # Critical fix: check AFTER install, BEFORE activation
    if not autogen_slconf.exists():
        raise RuntimeError(
            f"{autogen_slconf} not found after SLT install. "
            "SLT did not generate the expected toolchain config."
        )

    # ------------------------------------------------------------------
    # Activate SLT toolchain paths from autogen/pkg.slconf
    # ------------------------------------------------------------------
    tool_dirs = parse_tool_paths(autogen_slconf)
    normalized_paths = normalize_tool_paths(tool_dirs)
    env = prepend_path(env0, normalized_paths)

    verify_required_tools(env, ["python3", "cmake", "ninja", "java", "slc"])
    log("SLT toolchain activated")
